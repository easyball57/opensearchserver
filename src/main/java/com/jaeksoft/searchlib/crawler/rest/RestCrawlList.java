...
public class RestCrawlList implements XmlWriter.Interface {
	...
	public static RestCrawlList fromXml(RestCrawlMaster crawlMaster, File file)
		...
		if (rootNode == null)
			return crawlList;
		NodeList nodes = xpp.getNodeList(rootNode, RestCrawlItem.REST_CRAWL_NODE_NAME);
		if (nodes == null)
			return crawlList;
		for (int i = 0; i < nodes.getLength(); i++) {
			RestCrawlItem crawlItem = new RestCrawlItem(crawlMaster, xpp, nodes.item(i));
			if (crawlItem != null)
